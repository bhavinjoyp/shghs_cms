...
export default function EditNewsPage() {
        ...
        async function fetchData() {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    setUploadedImages(data.images_videos || []);
                    setThumbnail(data.thumbnail || '');
                } else {
                    setError(result.error || 'Could not load news article.');
                }
            } catch (e) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await fetch(`/api/news/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
            const result = await res.json();
            if (result.success) {
                router.push('/news');
            } else {
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div className="bg-white rounded-lg shadow-md p-8">
                        ...
                        {uploadedImages.length < 15 ? (
                            <ImageUpload onImageUploaded={handleImageUploaded} onError={handleUploadError} multiple maxFiles={15 - uploadedImages.length} title={formData.title || 'news-article'} mediaType="news" />
                        ) : (
